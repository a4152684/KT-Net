...
class BaseAgent(object):
    ...
    def load_ckpt(self, config):
        ...
        load_path = config.pretrain_ae_path
...
class GANzEAgent(object):
    ...
    def set_loss_function(self):
        ...
        self.criterionL2 = nn.MSELoss().cuda()
        self.criterionSL1 = nn.SmoothL1Loss().cuda()
    ...
    def set_optimizer(self, config):
        ...
        self.optimizer_D = torch.optim.Adam(self.netD.parameters(), lr=2*config.lr, betas=(config.beta1_gan, 0.999))
    ...
    def load_ckpt(self, config):
        """load checkpoint from saved checkpoint"""
        load_path = config.pretrain_gan_path
        if not os.path.exists(load_path):
            raise ValueError("Checkpoint {} not exists.".format(load_path))
